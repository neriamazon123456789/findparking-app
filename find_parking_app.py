...
st.text_input("🔍 כתובת לבדיקה", key="כתובת_קלט")
...
# הצגת התראה ב־18:30 רק אם ביקש ונמצא בתל אביב
עכשיו = datetime.datetime.now()
if (
    st.session_state["רוצה_תזכורת"] and
    "תל אביב" in st.session_state.get("כתובת", "") and
...
if "התראה_18_30" not in st.session_state or "תאריך_התראה" not in st.session_state or st.session_state["תאריך_התראה"] != תאריך_היום:
    st.session_state["התראה_18_30"] = False
    st.session_state["תאריך_התראה"] = תאריך_היום

# 🏁 כותרת והזנת כתובת
st.markdown("## 🅿️ FindParking – בדיקת כתובת חניה")
st.text_input("🔍 כתובת לבדיקה", key="כתובת")

# 🔘 כפתור הפעלת תזכורת
if st.button("🔔 הפעל תזכורת ל־18:30 להזיז את הרכב"):
    st.session_state["רוצה_תזכורת"] = True
    st.success("🔔 תזכורת הופעלה! נשלח הודעה ב־18:30 אם בוצעה בדיקה על תל אביב")
...
if st.button("📍 בדוק"):
    ...
    # בודקים אם מדובר בכתובת בת"א
    אם_תלאביב = "תל אביב" in כתובת
    אם_רוצה = st.session_state["רוצה_תזכורת"]

    if אם_תלאביב and אם_רוצה:
        st.session_state["ביקש_תזכורת"] = True

# 🛎️ התראת 18:30 (רק אם נבדק יעד בת"א והמשתמש ביקש)
if (
...
ד = (st.session_state["זמן_שניות"] % 3600) // 60
שנ = st.session_state["זמן_שניות"] % 60
st.markdown(f"## ⏱️ זמן: {ש:02}:{ד:02}:{שנ:02}")
