...
כתובות_כחול_לבן = [f"משה דיין {x} תל אביב" for x in list(range(1, 86)) if x not in [15,17,19] + [3,4,6,8,9,12,13,20,33]]
...
col1, col2, col3 = st.columns(3)
with col1:
    st.markdown(f"[📲 שלח בוואטסאפ](https://wa.me/?text=מצאתי%20אפליקציה%20למצוא%20חניה!%20{קישור})",
with col2:
