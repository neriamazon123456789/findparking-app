...
if st.button("📍 בדוק"):
    מצב_לפני = st.session_state.get("טיימר_פעיל", False)
    שניות_לפני = st.session_state.get("זמן_שניות", 0)

    כתובת = תקן_עיר_בכתובת(st.session_state["כתובת"]).strip().replace(",", "")
...
ש = st.session_state.get("זמן_שניות", 0) // 3600
ד = (st.session_state.get("זמן_שניות", 0) % 3600) // 60
שנ = st.session_state.get("זמן_שניות", 0) % 60
